...
namespace CFD_API.SignalR
{
    public class PositionReportTicker
    {
        ...
        private static readonly Lazy<PositionReportTicker> _instance =
            new Lazy<PositionReportTicker>(() => new PositionReportTicker(GlobalHost.ConnectionManager.GetHubContext<PositionHub>().Clients));
            ...
            while (true)
            {
                ...
                if (_subscription.Count > 0)
                {
                    ...
                    try
                    {
                        ...
                        foreach (var pair in dicUserPositionReports)
                        {
                            Clients.Group(pair.Key).p(pair.Value.Select(report =>
                            {
